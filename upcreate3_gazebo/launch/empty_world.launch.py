...
ARGUMENTS = [
    DeclareLaunchArgument('spawn_delay', default_value='5.0',
                          description='Seconds to wait after launching gazebo client to spawn the robot.'),
...
def generate_launch_description():
    ...
    pkg_upcreate_description = get_package_share_directory('upcreate3_description')

    # Set ignition resource path
    gz_resource_path = SetEnvironmentVariable(name='GAZEBO_MODEL_PATH', 
                                                value=[
                                                    EnvironmentVariable('GAZEBO_MODEL_PATH',default_value=''),
                                                    '/usr/share/gazebo-11/models/:',str(Path(pkg_irobot_create_description).parent.resolve())+':'+str(Path(pkg_upcreate_description).parent.resolve())
                                                ])
    ...
    robot_description_launch_file = PathJoinSubstitution(
        [pkg_upcreate_description, 'launch', 'description.launch.py'])
    ...
    spawn_robot = Node(
        package='gazebo_ros',
        executable='spawn_entity.py',
        name='spawn_upcreate3',
        arguments=['-entity','upcreate3',
    ...
    gazebo_event = RegisterEventHandler(
                        OnProcessStart(
                            ...
                            on_start=[
                                ...
                                TimerAction(
                                    period=spawn_delay,
                                    actions=[robot_description,spawn_robot,create3_nodes],
                                )
                            ]           
                        )
                    )
